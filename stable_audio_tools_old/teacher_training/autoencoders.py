...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        if self.use_disc:
            if loss_config['discriminator']['type'] == 'oobleck':
                ...
                self.discriminator = EncodecDiscriminator(in_channels=7, out_channels=7, **loss_config['discriminator']['config']) # discriminator on all channels...?
    ...
    def training_step(self, batch, batch_idx):
        ...
        if reals.shape[1] == 7: # remove audio before filtering if data sample contains audio
            ...
            reals = torch.cat((audio, real_emg), dim=1) # overwrite reals
            encoder_input = reals
            print('stacked encoder_input:', encoder_input.shape, flush=True)
            
        elif reals.shape[1] == 6: # filter everything if only emg is input
            real_emg = reals
        ...
        elif reals.shape[1] == 1: # if dataset is only audio, filter nothing
            encoder_input = reals
...
class AutoencoderDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            # ===== CHANGES FOR EMG+AUDIO --> EMG+AUDIO BRANCH TRAINING ===== 
            if demo_reals.shape[1] == 7: # remove audio before filtering if data sample contains audio
                ...
                demo_reals = torch.cat((audio, real_emg), dim=1) # overwrite reals
