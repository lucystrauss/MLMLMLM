...
from .utils import create_optimizer_from_config, create_scheduler_from_config, log_audio, log_image, log_metric, log_point_cloud, logger_project_name#, log_pitch
...
from ..models.factory import Filter_EMG

def log_pitch(logger, key, pitch_data):
    if isinstance(logger, WandbLogger):
        logger.experiment.log({key: wandb.plot.line(pitch_data, "timestep", "pitch", title=key)})
    elif isinstance(logger, CometLogger):
        print('comet logger of pitch not yet implemented.', flush=True)   

emg_filter = Filter_EMG(sr=44100, cutoff_freq=50)

def match_sr(src_latent, tgt_latent):

    src_len = src_latent.size(2)
    tgt_len = tgt_latent.size(2)
    ...
    if src_len < tgt_len:
        ...
        # Upsample target (rare, but handles it)
        tgt_latent = F.interpolate(tgt_latent, size=src_len, mode='linear', align_corners=True)
    
    return src_latent, tgt_latent
    
    

def detect_pitch_full_length(waveform, sample_rate):
    """
    Detect pitch and interpolate to match original waveform length.
    
    Args:
        waveform: Tensor of shape (batch_size, 1, seq_length)
        sample_rate: Sample rate in Hz
    
    Returns:
        pitch: Tensor of shape (batch_size, 1, seq_length) - same as input
    """
    batch_size, channels, seq_length = waveform.shape
    
    # Squeeze channel dimension for pitch detection
    waveform_2d = waveform.squeeze(1)  # (batch_size, seq_length)
    
    # Detect pitch with default values
    pitch = AudioFunc.detect_pitch_frequency(
        waveform_2d,
        sample_rate
    )  # (batch_size, frames)
    
    # Add channel dimension back
    pitch = pitch.unsqueeze(1)  # (batch_size, 1, frames)
    
    # Interpolate to match original length
    pitch_full = F.interpolate(
        pitch,
        size=seq_length,
        mode='linear',
        align_corners=False
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
            ...
            sample_rate=44100, # should match training dataset audio. NB that in my dataset, emg is 22050 and audio is 44100, time aligned
            loss_config: Optional[dict] = None,
            eval_loss_config: Optional[dict] = None,
            optimizer_configs: Optional[dict] = None,
            lr: float = 1e-5, # changed from 1e-4 on 13 June 2025
            warmup_steps: int = 0,
            warmup_mode: Literal["adv", "full"] = "adv",
            encoder_freeze_on_warmup: bool = False,#True,#False,
            use_ema: bool = True,
            ema_copy = None,
        ...
        self.encoder_freeze_on_warmup = False,#True#encoder_freeze_on_warmup
        self.lr = lr
        ...
        if self.autoencoder.out_channels == 2:
            print('ERROR: self.autoencoder.out_channels == 2', flush=True)
        ...
        if self.use_disc:
            if loss_config['discriminator']['type'] == 'oobleck':
                ...
                self.discriminator = EncodecDiscriminator(in_channels=8, out_channels=8, **loss_config['discriminator']['config']) # discriminator on all channels...?
    ...
    def training_step(self, batch, batch_idx):
        ...
        #if len(reals.shape) == 2:
            #reals = reals.unsqueeze(1) # why? Does unsqueeze happen in encode Mono() but not Stereo() ? No... I think we don't need this for EMG dataset
        ...
        # ===== CHANGES FOR EMG --> AUDIO BRANCH TRAINING ===== This is where we separate audio & EMG into their own variables. EMG goes through encoder, but audio is kept for the reconstruction loss function...
        
        #print('reals shape', reals.shape, flush=True)
		
        #encoder_input = reals[:, 1:] # removing the audio channel from the loaded data sample
        #print('encoder input', encoder_input.shape, flush=True)
		
        #reals = reals[:, 0:1] # reals is now overwritten with only the mono audio channel
        #print('reals (audio only)', reals.shape, flush=True)
		
		# =================================================
		
		
        # ===== CHANGES FOR EMG+AUDIO --> EMG+AUDIO BRANCH TRAINING ===== 
        real_emg = reals[:,1:,:] # slice emg from emg+audio tensor; 
        print('real_emg shape before downsampling:', real_emg.shape, flush=True)
        real_emg = emg_filter(real_emg) # 50Hz filtering before downsampling
        
        real_emg = real_emg[:, :, ::2] # remove every second EMG sample (downsample)
        
        print('real_emg shape after downsampling:', real_emg.shape, flush=True)
        
        real_emg, _ = match_sr(real_emg, reals[:,1:,:])
        print('real_emg shape after upsampling:', real_emg.shape, flush=True)
        # recombine audio and resampled emg
        
        
        #print('reals shape', reals.shape, flush=True)
        #encoder_input = reals[:, 1:]
        #reals = reals[:, 1:]
        #print('encoder input', encoder_input.shape, flush=True)
		
        audio = reals[:,0:1,:] # audio is now only the mono audio channel
        print('reals (audio only)', audio.shape, flush=True)
        
        pitch_detect = detect_pitch_full_length(audio, 44100)
        print('pitch_detect:', pitch_detect.shape, pitch_detect, flush=True)
        
        reals = torch.cat((audio, real_emg, pitch_detect), dim=1) # overwrite reals with pitch detection at the end so as not to mess up plotting later on
        encoder_input = reals
        print('stacked encoder_input:', encoder_input.shape, flush=True)
        ...
        decoded = self.autoencoder.decode(latents)
        print('decoded tensor', decoded.shape, flush=True)

        #Trim output to remove post-padding
        decoded, reals = trim_to_shortest(decoded, reals)

        loss_info["decoded"] = decoded
        loss_info["reals"] = encoder_input

        if self.autoencoder.out_channels == 2:
            print('DEBUG ERROR: autoencoder.out_channels == 2')
        ...
        if self.use_disc:
            print('use_disc', flush=True)
            if self.warmed_up:
                loss_dis, loss_adv, feature_matching_distance = self.discriminator.loss(reals=encoder_input, fakes=decoded)
            else:
                loss_adv = torch.tensor(0.).to(reals)
                feature_matching_distance = torch.tensor(0.).to(reals)

                if self.warmup_mode == "adv":
                    loss_dis, _, _ = self.discriminator.loss(reals=reals, fakes=decoded) # discriminator only on audio
                else:
        ...
        if use_disc:
            #print('use_disc', flush=True)
            print(f'Batch {batch_idx}: use_disc = True', flush=True)
        ...
        # Train the generator
            ...
            print(f'Batch {batch_idx}: training generator (use_disc = False)', flush=True)
...
class AutoencoderDemoCallback(pl.Callback):
    def __init__(
        ...
        self.demo_dataset = demo_dl.dataset # from claude option 2
        self.batch_size = demo_dl.batch_size # from claude option 2
        self.num_workers = demo_dl.num_workers # from claude option 2
        self.collate_fn = demo_dl.collate_fn # from claude option 2
        self._create_new_iterator() # from claude option 2
        
        
        #self.original_demo_dl = deepcopy(demo_dl) # from claude
        #self.demo_dl = iter(self.original_demo_dl) # from claude
    ...
    def _create_new_iterator(self):
        """Create a new dataloader and iterator"""
        ...
        new_dl = DataLoader(
            ...
            shuffle=True  # Add shuffle if you want different samples each time
        )
        self.demo_dl = iter(new_dl)


    @rank_zero_only
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
    
        #print(f"DEBUG: Step {trainer.global_step}, last_demo_step: {self.last_demo_step}")
        #print(f"DEBUG: Should demo: {trainer.global_step % self.demo_every == 0}")
        ...
        print(f"DEBUG: Running demo at step {trainer.global_step}")
        
        self.last_demo_step = trainer.global_step
        module.eval()
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        print('=========DEBUG: CUDA MEMORY', torch.cuda.mem_get_info(device), flush=True)
        try:
            ...
            try:
                ...
                #self.demo_dl = iter(deepcopy(self.original_demo_dl))
            ...
            if demo_reals.shape[0] > self.max_demos: # only demo the first of the batch
                demo_reals = demo_reals[:self.max_demos,...]

            # ====== CHANGES FOR EMG --> AUDIO VAE ====== separate reals from encoder_input just like you have done in the training iteration loop above :)
            
            
            
            # ===== CHANGES FOR EMG+AUDIO --> EMG+AUDIO BRANCH TRAINING ===== 
            real_emg = demo_reals[:,1:,:] # slice emg from emg+audio tensor; 
            print('real_emg shape before downsampling:', real_emg.shape, flush=True)
            real_emg = emg_filter(real_emg) # 50Hz filtering before downsampling
            ...
            real_emg, _ = match_sr(real_emg, demo_reals[:,1:,:])
            print('real_emg shape after upsampling:', real_emg.shape, flush=True)
            
            
            audio = demo_reals[:,0:1,:] # audio is now only the mono audio channel
            print('reals (audio only)', audio.shape, flush=True)
        
            pitch_detect = detect_pitch_full_length(audio, 44100)
        
            demo_reals = torch.cat((audio, real_emg, pitch_detect), dim=1) # overwrite reals with pitch detection at the end so as not to mess up plotting later on
            ...
            print('demo_reals shape', demo_reals.shape, flush=True)

            encoder_input = demo_reals#[:, 1:]
            #demo_reals = demo_reals[:, 1:]
            print('demo encoder input', encoder_input.shape, flush=True)
            encoder_input = encoder_input.to(module.device)
            
            #audio_reals = demo_reals[:, 0:1].to(module.device) # demo_reals is now overwritten with only the mono audio channel
            #print('demo reals (audio only)', demo_reals.shape, flush=True)
            ...
            if module.discriminator is not None:
                print("DEBUG: module.discriminator is not None")
                ...
                print('reals signal shape', reals_signal.shape, flush=True)
                print('fakes signal shape', fakes_signal.shape, flush=True)
            ...
            #audio_fakes = fakes[:, 0:1] # only plot and play audio fakes... TODO: also visualize emg reconstructions
            #print('>>>>>>>>> audio_reals, audio_fakes:', audio_reals.device, audio_fakes.device, flush=True)
            #Interleave reals and fakes
            reals_fakes = rearrange([demo_reals[:, 0:1], fakes[:, 0:1]], 'i b d n -> (b i) d n')
            ...
            print('>>>>>demo reals, fakes:', demo_reals.shape, demo_reals.device, fakes.shape, fakes.device)
            _, channels, _ = demo_reals.shape
            for channel_idx in range(channels -1):
                # Step 1: Extract single channel
                real_ch = demo_reals[:, channel_idx, :]    # [batch, length]
                fake_ch = fakes[:, channel_idx, :]    # [batch, length]
        
                # Step 2: Add dimension back and stack
                real_ch = real_ch.unsqueeze(1)              # [batch, 1, length]
                fake_ch = fake_ch.unsqueeze(1)              # [batch, 1, length]
        
                # Step 3: Apply original rearrange operations
                stacked = torch.stack([real_ch, fake_ch], dim=0)  # [2, batch, 1, length]
                ch_reals_fakes = rearrange(stacked, 'i b d n -> (b i) d n')  # [batch*2, 1, length]
                ch_reals_fakes = rearrange(ch_reals_fakes, 'b d n -> d (b n)')  # [1, batch*2*length]
                
                # Step 4: Convert and clamp
                ch_reals_fakes = ch_reals_fakes.to(torch.float32).clamp(-1, 1).mul(32767).to(torch.int16).cpu()
        
                # Step 5: Log spectrogram
                log_image(trainer.logger, f'channel_{channel_idx}', audio_spectrogram_image(ch_reals_fakes))



            '''Plot pitch detection results.'''
            reals_fakes_pitch = rearrange([demo_reals[:, 6:7], fakes[:, 6:7]], 'i b d n -> (b i) d n')
            # Put the demos together
            reals_fakes_pitch = rearrange(reals_fakes_pitch, 'b d n -> d (b n)')

            #reals_fakes_pitch = reals_fakes_pitch[0, 0].cpu().numpy()  # Shape: (seq_len,)
            reals_fakes_pitch = reals_fakes_pitch[0].cpu().numpy()
            # Create x-axis as timesteps (0 to seq_len-1)
            timesteps = list(range(len(reals_fakes_pitch)))
    
            # Create data for wandb table
            reals_fakes_pitch = [[x, y] for (x, y) in zip(timesteps, reals_fakes_pitch)]
            reals_fakes_pitch = wandb.Table(data=reals_fakes_pitch, columns=["timestep", "pitch"])
    
            # Log to wandb
            log_pitch(trainer.logger, 'pitch', reals_fakes_pitch)
