...
#from mappers.mapper_models import match_sr
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
            ...
            sample_rate=44100, # TODO @Lucy: NB to check sample_rate in ./training/autoencoders.py as well... have we been resampling or calculating STFT losses in 48000Hz this entire time? If so... oops...
        ...
        if self.use_disc:
            if loss_config['discriminator']['type'] == 'oobleck':
                ...
                self.discriminator = EncodecDiscriminator(in_channels=6, out_channels=6, **loss_config['discriminator']['config'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ===== CHANGES FOR EMG+AUDIO --> EMG+AUDIO BRANCH TRAINING ===== This is where we make a separate audio for reconstruction plotting...
        ...
        audio = reals[:, 0:1] # reals is now overwritten with only the mono audio channel
        print('reals (audio only)', reals.shape, flush=True)
        ...
        # ===== at this point, encoder_input should be shape: "DEBUG: encoder input shape torch.Size([batch_size, channels, sample_length])" =====
        
        print('>>>>>>>>> encoder input dtype:', encoder_input.dtype, flush=True)
        #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        #emg_filter = Filter_EMG(sr=44100, cutoff_freq=50)#.to(device) # TODO: make this automatic
        #encoder_input_filt = emg_filter(encoder_input)
        #encoder_input_filt = encoder_input_filt[:, :, ::2]
        #encoder_input = F.interpolate(encoder_input_filt, size=encoder_input.size(2), mode='linear', align_corners=True)
        
        #del encoder_input_filt
...
class AutoencoderDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            #reals_fakes = rearrange([audio_reals, audio_fakes], 'i b d n -> (b i) d n')
