...
class Filter_EMG:
    def __init__(self, sr=22050, cutoff_freq=50, bandwidth=4):
        self.sr = sr
        self.cutoff_freq = cutoff_freq
        self.bandwidth = bandwidth
    
    def __call__(self, batch_tensor):
        batch_size, n_channels, seq_len = batch_tensor.shape
        batch_tensor = batch_tensor.clone()
        
        for batch_idx in range(batch_size):
            for ch in range(1, n_channels):
                batch_tensor[batch_idx, ch] = self._notch_filter_fft(
                    batch_tensor[batch_idx, ch]
                )
        
        return batch_tensor
    
    def _notch_filter_fft(self, signal):
        """Apply notch filter using FFT"""
        X = torch.fft.fft(signal)
        freqs = torch.fft.fftfreq(len(signal), 1/self.sr).to(signal.device)
        abs_freqs = torch.abs(freqs)
        center_freq = self.cutoff_freq
        
        # Proper Butterworth notch: attenuate based on distance from center
        freq_diff = torch.abs(abs_freqs - center_freq)
        
        # Butterworth notch filter response
        # At 50Hz (freq_diff=0): response = 0 (maximum attenuation)
        # Far from 50Hz: response ≈ 1 (minimal attenuation)
        bandwidth_term = torch.tensor(self.bandwidth/2, device=signal.device, dtype=signal.dtype)
        butterworth_notch = torch.pow(freq_diff, 8) / (torch.pow(freq_diff, 8) + torch.pow(bandwidth_term, 8))
        
        X_filtered = X * butterworth_notch
        #print('DEBUG: emg filtered.', flush=True)
        return torch.fft.ifft(X_filtered).real
