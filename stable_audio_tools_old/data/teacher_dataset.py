...
def worker_init_fn(worker_id):
    """Properly seed each DataLoader worker for reproducible randomness"""
...
class SampleDataset(torch.utils.data.Dataset):
    ...
    def load_file(self, filename):
        ...
        #if in_sr != self.sr:
         #   print('resampling input', flush=True)
           # resample_tf = T.Resample(in_sr, self.sr)
           # audio = resample_tf(audio)
        print('sample shape', audio.shape)
        return audio
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            for custom_md_path in self.custom_metadata_fns.keys():
                ...
                if "__audio__" in info:
                    for audio_key, audio_value in info["__audio__"].items():
                        ...
                        if self.encoding is not None:
                            print('>>>>>>>>> doing encoding!', flush=True)
			...
			# TODO: add samplerate conversion here?
