...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        if self.use_disc:
            if loss_config['discriminator']['type'] == 'oobleck':
                ...
                self.discriminator = EncodecDiscriminator(in_channels=6, **loss_config['discriminator']['config'])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if len(reals.shape) == 2:
            reals = reals.unsqueeze(1) # add batch dimension if it is absent
            
        reals = reals[:, 0:1, :] # remove emg channels from paired dataset    
    ...
    def training_step(self, batch, batch_idx):
        reals, _ = batch
        reals = reals[:, 1:, :]
        print('starting training step reals shape', reals.shape, flush=True)
        ...
        encoder_input = reals
        print('DEBUG: encoder_input shape', encoder_input.shape, flush=True)


        loss_info["encoder_input"] = encoder_input

        data_std = encoder_input.std()
        #print('DEBUG: encoder input shape', encoder_input.shape)
        if self.warmed_up and self.encoder_freeze_on_warmup:
            #with torch.no_grad():
                #latents, encoder_info = self.autoencoder.encode(encoder_input, return_info=True)
                #print('DEBUG: encoder frozen')
            latents, encoder_info = self.autoencoder.encode(encoder_input, return_info=True)
            print('DEBUG: encoder NOT frozen')
        else:
            print('encoder_input shape just before encoding', encoder_input.shape)
            latents, encoder_info = self.autoencoder.encode(encoder_input, return_info=True)
            print('DEBUG: encoder NOT frozen')
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            try:
                ...
                demo_reals = demo_reals[:, 1:, :] # remove audio channel
            ...
            #demo_reals = demo_reals[:, 0:1, :]

            encoder_input = demo_reals[0:1, :, :] # only demo first batch
