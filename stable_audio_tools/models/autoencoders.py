...
from .factory import Filter_EMG
...
class OobleckDecoder(nn.Module):
    ...
    def forward(self, x):
        print('oobleck decoder', flush=True)
        return self.layers(x)
...
class AudioAutoencoder(nn.Module):
    ...
    def encode(self, audio, skip_bottleneck: bool = False, return_info=False, skip_pretransform=False, iterate_batch=False, **kwargs):
        ...
        #print('DEBUG: audio shape:', audio.shape)
        '''if audio.shape[1] > 2:
            #print('DEBUG: applying 50Hz filter', flush=True)
            emg_filter = Filter_EMG(sr=22050, cutoff_freq=50)#.to(device) # TODO: make this automatic
            audio = emg_filter(audio)'''
            
        #else:
            #print('DEBUG: NOT applying 50Hz filter,', flush=True)

        if self.pretransform is not None and not skip_pretransform:
            print('======== implementing pretransforms ========', flush=True)
        ...
        if self.encoder is not None:
            print('========== audio shape just before encoder:', audio.shape, flush=True)
            if iterate_batch:
                latents = []
                for i in range(audio.shape[0]):
            ...
            latents = audio
        print('========= latents after encoder before bottleneck:', latents.shape, flush=True)
        info["pre_bottleneck_latents"] = latents

        if self.bottleneck is not None and not skip_bottleneck:
            print('======== implementing encoder bottleneck', flush=True)
            # TODO: Add iterate batch logic, needs to merge the info dicts
            latents, bottleneck_info = self.bottleneck.encode(latents, return_info=True, **kwargs)
            print('========= latents after encoder bottleneck', latents.shape, flush=True)
    ...
    def decode(self, latents, skip_bottleneck: bool = False, iterate_batch=False, **kwargs):
        ...
        if self.bottleneck is not None and not skip_bottleneck:
            print('======== implementing decoder bottleneck', flush=True)
            if iterate_batch:
                decoded = []
        ...
        print('======== latents shape just before going into decoder', latents.shape, flush=True)
        if iterate_batch:
            print('iterate batch', flush=True)
            decoded = []
            for i in range(latents.shape[0]):
                decoded.append(self.decoder(latents[i:i+1]))
            decoded = torch.cat(decoded, dim=0)
        else:
            print('trying to decode', flush=True)
    ...
    def decode_tokens(self, tokens, **kwargs):
        ...
        '''
        print('about to assert', flush=True)
        assert isinstance(self.bottleneck, DiscreteBottleneck), "decode_tokens only works with discrete autoencoders"
        print('about to decode_tokens', flush=True)
        latents = self.bottleneck.decode_tokens(tokens, **kwargs)
        latents = latents.permute(0, 2, 1)
        print('about to decode', flush=True)
        output = self.decode(latents, **kwargs)
        return output
    ...
    def preprocess_audio_for_encoder(self, audio, in_sr):
        ...
        '''
        print('========preprocessing audio=========', flush=True)
    ...
    def encode_audio(self, audio, chunked=False, overlap=32, chunk_size=128, **kwargs):
        ...
        '''
        print('========== audio shape after preprocessing:', audio.shape, flush=True)
        if not chunked:
