...
def sample_flow_pingpong_cont(model, x, mask=None, steps=None, sigma_max=1, sigmas=None, callback=None, dist_shift=None, **extra_args):
    """Draws samples from a model given starting noise. Ping-pong sampling for distilled models"""
    
    assert steps is not None or sigmas is not None, "Either steps or sigmas must be provided"
    
    mask = extra_args.pop('mask', mask)
    original = extra_args.pop('original', None)
    # Extract mask and original if provided
    #mask = extra_args.get('mask', None)
    #original = extra_args.get('original', None)
    
    print('mask shape:', mask.shape)    
    if mask is not None and original is not None:
    
        if mask.shape[-1] != original.shape[-1]:
            print(f"BEFORE interpolation:")
            print(f"  mask shape: {mask.shape}")
            print(f"  mask preserve (0): {(mask == 0).sum()}")
            print(f"  mask generate (1): {(mask == 1).sum()}")
    ...
    for i in trange(len(t) - 1, disable=False):
        ...
        # Apply mask to preserve original regions
        if mask is not None and original is not None:
        
            if i == 0:  # Only print on first iteration to avoid spam
                print(f"DEBUG MASKING (step {i}):")
                ...
                # Check what the masking actually does
                preserved_part = original * (1 - mask)
                generated_part = x * mask
            
                print(f"  preserved_part max: {preserved_part.abs().max()}")
                print(f"  generated_part max: {generated_part.abs().max()}")
                print(f"  original max: {original.abs().max()}")
                print(f"  x max before masking: {x.abs().max()}")
        

            #print(f"DEBUG: Applying mask - original shape: {original.shape}, mask shape: {mask.shape}")
            #print(f"DEBUG: Mask values - min: {mask.min()}, max: {mask.max()}")
            #print(f"DEBUG: Preserve regions (mask=0): {(mask == 0).sum()}")
            #print(f"DEBUG: Generate regions (mask=1): {(mask == 1).sum()}")

            #print('mask and original audio have been provided.')
...
def sample_k(
    ...
    if is_k_diff:
        ...
        if init_data is not None:
            print('set the initial latent to the init_data, and noise it with initial sigma')
            x = init_data + noise
        else:
            # SAMPLING
            print('set the initial latent to noise')
            x = noise
        ...
        if sampler_type == "k-heun":
            ...
            print('sampler type is dpmpp-2m')
...
def sample_rf(
    ...
    if init_data is not None:
        ...
        # VARIATION
        print('Interpolate the init data and the noise for init audio')
        x = init_data * (1 - sigma_max) + noise * sigma_max
        #x = init_data #* 0.5 + noise * 0.5
    ...
    elif sampler_type == "pingpong_cont":
        print('pingpong_cont')
        extra_args_with_data = {**extra_args, 'mask': mask, 'original': original}
        return sample_flow_pingpong_cont(model_fn, x, sigmas=t, sigma_max=sigma_max, callback=callback, **extra_args_with_data)
