...
class RVQBottleneck(DiscreteBottleneck):
    ...
    def decode_tokens(self, indices, **kwargs):
        # Ensure dtype and device
        indices = indices.to(torch.long).to(self.quantizer.codebooks.device)

        # Clamp to valid range
        codebook_size = self.quantizer.codebooks.shape[1]
        indices = indices.clamp(0, codebook_size - 1)

        # Optional: log anomalies
        bad_low = (indices < 0).sum().item()
        bad_high = (indices >= codebook_size).sum().item()
        if bad_low or bad_high:
            print(f"Clamped {bad_low} low and {bad_high} high indices", flush=True)

        # Now safe to call into quantizer
        latents = self.quantizer.get_output_from_indices(indices)
        return latents
...
class RVQVAEBottleneck(DiscreteBottleneck):
    ...
    def decode_tokens(self, codes, **kwargs):
        print('entered decode tokens', flush=True)
        latents = self.quantizer.get_output_from_indices(codes)
        print('got codes; about to permute', flush=True)
